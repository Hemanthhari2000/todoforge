...
from todoforge.services.app_state import SpaceStateService
from todoforge.services.space import SpaceService
from todoforge.utils.config import todo_config
from todoforge.utils.constants import DEFAULT_TODO_CONFIG, DEFAULT_TODO_FOLDER


class SpaceCommand:
    def __init__(self) -> None:
        self.service = SpaceService()
        self.app_state_service = SpaceStateService()

    def add(self, space_name: str):
        """
        ...
        """
        try:
            space = SpaceModel(name=space_name)
            self.service.add(space)

        except (ValueError, OSError) as e:
            print(f"[red]{e}[/red]")
            raise typer.Exit(code=1)

        print(f"Space [green]{space.name}[/green] has been created successfully")
        ...
        Example:
            $ todoforge ls
                * personal
                work
        """
        spaces_tuple = self.service.get_all()
        if spaces_tuple:
            spaces, current_space = spaces_tuple
            for s in spaces:
                prefix = "[green]*[/green]" if s.name == current_space else " "
                print(f"{prefix} {s.name}")

    def switch(self, space_name: str):
        """
    ...
    def remove(self, space_name: str):
        """
        Removes a space.
        ...
        self.service.remove(space_name)
        print(f"[green]{space_name}[/green] removed")


app = typer.Typer()
space_cmd = SpaceCommand()

app.command()(space_cmd.add)
app.command()(space_cmd.ls)
app.command()(space_cmd.switch)
app.command()(space_cmd.rename)
app.command()(space_cmd.remove)
